...
- **连续加注对手门槛逐级上调（v1.16.8 修正）**：已看牌对手连续 raise 时，旧实现只按赔率 break-even 反推门槛（`_opponent_hand_threshold`），连续 raise 的强度信号被低估——uniform[门槛,1.0] 假设把约 77% 权重放在「比我方同花小的牌仍连加」上，胜率虚高 → 摸到同花死追对手连加输钱。现 `_update_round_tracker` 累计各对手本局 raise 次数（`_RoundTracker.opponent_raise_counts`，lastAction 变化且含「加」才计一次），`_seen_opponent_ranges` 对加注对手按次数逐级上调门槛（每次关掉剩余区间一半 β=0.5，强于决策树推演的 +25%/次——已确认的连续 raise 是事实信号），且上调后不被画像历史弱牌加注下四分位拉低（取 max）。**强制摊牌阶段（phase=showdown）不升级**：实测该阶段 `actions` 只剩 fold/raise/showdown，raise 是唯一「继续」动作，对手被迫每轮 raise，不代表牌强（单挑实测 6-8 轮 callBet 3000→24000）。（v1.16.16 已撤销摊牌豁免：弱牌玩家有 fold/showdown 便宜出口不会白 raise，摊牌 raise 照常升级。）
...
- **对手牌力门槛改按深度 EV 反推（v1.16.20）**：旧版单步 breakeven `call/(pot+call)` 隐含「跟注这手就结束」，忽略后续轮次追平与终局 showdown 成本——但对手上牌若 `open` 我方至少开牌才能结束，只上不开则我方提 `open` 他要 `showdown` 才会结束（用户 2026-08-08：这些金额也要算进去才能算出来 ev）。现 `_combined_opponent_threshold`/`_opponent_hand_threshold` 在 `depth>0` 时走 `_opponent_deep_threshold`：站在对手视角（已看牌、牌力门槛 t）递归推演 `zjh_terminal_depth` 轮（默认 2，与蒙牌 Terminal EV 决策树同配置），每轮时序为「对手先按 callBet 入池 → 我方行动」，我方 call/raise 按 `_self_action_probs` 加权（把我也作为一个用户查我方账号画像，`_train_opponent_actions` 不再跳过我方；无画像/不启用时回退必平跟），我方蒙牌下注半价（`self_seen=False`）；我方 raise 使 callBet 线性递增一注底注（实测 `ante→2×ante→3×ante`）；深度耗尽 = 强制摊牌，对手付最后一次 showdown 成本（=当时 callBet 单倍，含在入池），净值 = `胜率×底池−成本`。二分 `EV(t)=0` 求门槛，下界从 `max(seen_thresholds)` 起（门槛低于所有已看牌对手则胜率 0、EV 恒负）。因 `EV(1.0)=底池累计+我方入池>0` 恒正，合法快照总有解（单挑 peek 门槛 0.222→0.353、continue 0.588→0.645），None 仅由非法快照（seen 越界等防御校验）触发。v1.16.18 的 `min_peek` 下限保留参数向后兼容，决策侧不再传（深度反推天然更高）。**单步 breakeven 保留在记录/诊断侧**（`depth=0`），决策日志展示深度反推门槛。
- **主动开牌（open）无开关（v1.16.19）**：移除 `zjh_open_enabled` 配置，`_choose_action` 在「服务端授权 `open` 且最终实际胜率 < `zjh_open_max_win_rate`（默认 50%）」时直接主动开牌（开牌止损，EV 为正但胜率低时抢先比牌，比继续跟注省钱）。开关曾因配置漂移丢失导致主动开牌整体失效（用户 2026-08-08 反馈「开关弄丢后不再主动开牌输钱」），现只保留胜率阈值、去掉开关。**已看牌主动开牌**（本列表项）与**单挑蒙牌直接开牌**（`_heads_up_blind_action`，不看牌 showdown/open 比大小）是两条独立路径，后者不受胜率阈值影响。
