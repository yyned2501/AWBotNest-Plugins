...
**规则推断（结算文案反推，部分已实测）**：目标 10.5 点，超过即爆牌（玩家爆牌输掉下注、庄家爆牌玩家赢）；玩家点数大于庄家则赢，**平局算玩家输**；赢面有 1% 抽水（下注 100 赢 +99，`rakeTotal` 与庄家 delta 对应）；半点存在（实测点数 0.5/9.5 递增，J/Q/K 计 0.5 的经典十点半规则基本坐实）；有 AI 玩家参与（`player.ai`/source=ai）；**庄家爆牌局的 `dealerHandLabel` 不含「爆」字、直接显示超点点数（如「11点」，2026-08-17 实测）**，爆牌只能按点数 > 10.5 判定。**`lastResult` 只在 settled 阶段窗口内可见、翻篇即丢（2026-08-18 实测）**：快速局停牌后 5 秒内就开下一局报名，settled 窗口可能短于轮询间隔，错过就永久错过——结算入账不能只信 `lastResult`，v1.19.1 起报名记局号、翻篇后从 `history[]`（带完整 settlement）补扫，history 也没有时降级推送兜底。
