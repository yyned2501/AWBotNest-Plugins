...
类 21 点的庄家制牌戏，门户前端 `portal-games.js`（v20260806-19）驱动。**v1.17.0 已接入插件**（skyGame `games/tenhalf.py`：signup 报名 → 按爆牌概率 + 庄家画像 EV 决策要/停牌，kv 记累计战绩与庄家画像；只玩玩家位不开庄；默认关闭，POST 契约未实测故全防御性解析、失败按局去重通知）。
