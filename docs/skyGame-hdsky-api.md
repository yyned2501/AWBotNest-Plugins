...
类 21 点的庄家制牌戏，门户前端 `portal-games.js`（v20260806-19）驱动。**v1.17.0 已接入插件**（skyGame `games/tenhalf.py`：signup 报名 → 按爆牌概率 + 庄家画像 EV 决策要/停牌，kv 记累计战绩与庄家画像；只玩玩家位不开庄；默认关闭，POST 契约未实测故全防御性解析、失败按局去重通知；v1.18.0 起停牌阈值样本足够时完全由庄家画像推导：均点+0.5（平局算输）按爆率让利，夹 4～10；v1.19.0 起庄家画像按终局手牌张数分桶（张数靠轮询观察 players[].cardCount 按 roundId 暂存配对，结算只有点数没有张数），阈值优先用当前庄家张数对应桶推导）。
...
**规则推断（结算文案反推，部分已实测）**：目标 10.5 点，超过即爆牌（玩家爆牌输掉下注、庄家爆牌玩家赢）；玩家点数大于庄家则赢，**平局算玩家输**；赢面有 1% 抽水（下注 100 赢 +99，`rakeTotal` 与庄家 delta 对应）；半点存在（实测点数 0.5/9.5 递增，J/Q/K 计 0.5 的经典十点半规则基本坐实）；有 AI 玩家参与（`player.ai`/source=ai）；**庄家爆牌局的 `dealerHandLabel` 不含「爆」字、直接显示超点点数（如「11点」，2026-08-17 实测）**，爆牌只能按点数 > 10.5 判定。
