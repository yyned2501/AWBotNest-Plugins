...
类 21 点的庄家制牌戏，门户前端 `portal-games.js`（v20260806-19）驱动。**v1.17.0 已接入插件**（skyGame `games/tenhalf.py`：signup 报名 → 按爆牌概率 + 庄家画像 EV 决策要/停牌，kv 记累计战绩与庄家画像；只玩玩家位不开庄；默认关闭，POST 契约未实测故全防御性解析、失败按局去重通知；v1.18.0 起停牌阈值样本足够时完全由庄家画像推导：均点+0.5（平局算输）按爆率让利，夹 4～10；v1.19.0 起庄家画像按终局手牌张数分桶（张数靠轮询观察 players[].cardCount 按 roundId 暂存配对，结算只有点数没有张数），阈值优先用当前庄家张数对应桶推导；v1.19.1 起结算改用 history[] 补扫兜底，修复快速局结算推送丢失；v1.20.0 起画像阈值夹在 4～6.5 爆牌红线，早停赌庄家自爆；v1.21.0 起样本足够时改 EV 决策：按画像点数分布+爆率算停牌 EV，对比 52 张先验递推的要牌 EV（含五小 ×5）择优，庄家/我方五小直接停牌；notify 异常吞掉只记日志，断网不再打断轮询）。
...
**规则推断（结算文案反推，部分已实测）**：目标 10.5 点，超过即爆牌（玩家爆牌输掉下注、庄家爆牌玩家赢）；玩家点数大于庄家则赢，**平局算玩家输**；赢面有 1% 抽水（下注 100 赢 +99，`rakeTotal` 与庄家 delta 对应）；半点存在（实测点数 0.5/9.5 递增，J/Q/K 计 0.5 的经典十点半规则基本坐实）；有 AI 玩家参与（`player.ai`/source=ai）；**庄家爆牌局的 `dealerHandLabel` 不含「爆」字、直接显示超点点数（如「11点」，2026-08-17 实测）**，爆牌只能按点数 > 10.5 判定。**倍数机制（2026-08-18 实测发现）**：结算 `resultText` 带 ×N 倍数（如「下注 100 ×5」），「玩家/庄家 5 小」（5 张不爆）×5 坐实（如 #1931 +495/#1924 -500）；玩家爆牌局也出现过 ×2/×5（#1930/#1892/#1898），推测倍数由手牌张数成就驱动且全桌适用（具体规则待更多样本确认）；倍数放大爆牌代价，是 v1.20.0 爆牌红线 6.5 的依据。**`lastResult` 只在 settled 阶段窗口内可见、翻篇即丢（2026-08-18 实测）**：快速局停牌后 5 秒内就开下一局报名，settled 窗口可能短于轮询间隔，错过就永久错过——结算入账不能只信 `lastResult`，v1.19.1 起报名记局号、翻篇后从 `history[]`（带完整 settlement）补扫，history 也没有时降级推送兜底。
...
- 十点半：`POST /api/portal/tenhalf/start` 的开庄请求格式、抓牌点数构成细节与 AI 玩家行为仍可继续实测（join/hit/stand 已实测可用，插件 v1.17.0 起接入、v1.18.1 修复庄家爆牌识别）。
