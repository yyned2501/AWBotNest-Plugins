...
- 遛马动作：冷却拒绝时返回外层 `ok: true`、`result.code == "cooldown"`、`result.remainMs`、`result.message`（2026-08-14 实测文案「你的马刚遛过，2小时58分钟 后再来」，冷却约 3 小时而非旧记的 45 分钟），期间 `state.canWalk` 仍为 `true`。插件记下 `remainMs` 换算的到期时间退避，未到不再尝试；`cooldown` 不计入失败计数，连续真失败 3 次后跳过本轮。
